...
class notComplete extends StatefulWidget {
  const notComplete({super.key});

  @override
  State<notComplete> createState() => _CompleteTaskState();
}

class _CompleteTaskState extends State<notComplete> {
  ...
  Widget build(BuildContext context) {
    final allTask =
        dataBase.db.tasks.where((element) => element.status == false).toList();
    return Column(
      children: [
        Expanded(
                      ...
                      onChanged: (value) => {
                        setState(() {
                          tc.updateStatusTask(taskModel(
                              status: true, title: allTask[index].title));
                        }),
                        print(index),
                        print(value)
                      },
